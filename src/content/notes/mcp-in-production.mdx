...
MCP makes adding tools easy, which means teams add more of them, which raises the floor cost of all traffic including the calls that use none. I have written about [the tool-definition tax](/notes/tool-definition-tax) with the arithmetic; on most realistic workloads it lands between 8% and 25% of input cost.
...
**Return errors the model can act on.** `Customer 4471 not found — search by email instead` gives the model a different action. `Error 500` gives it nothing, so it retries the identical call, reasoning it may work this time. That is a large share of the [runaway loops](/tools/failure-taxonomy#agent-loops-forever) I find, and it is a tool design problem rather than an orchestration one.
