...
Then push further: what does the tool return *on failure*? `Error 500` gives the model nothing to change, so it retries the identical call. `Customer 4471 not found — search by email instead` gives it a different action. A large share of [runaway loops](/tools/ai-agent-failure-modes#agent-loops-forever) are caused by unactionable error messages, which is a tool design problem and free to fix at this stage.
...
This one decision removes a large class of [non-determinism](/tools/ai-agent-failure-modes#same-input-different-output) and makes most of the eval suite deterministic rather than needing a judge.
