...
What to cache: system prompts and tool definitions. Stable text, re-read on every call, and [tool definitions are billed on every request anyway](/notes/tool-definition-token-cost) so they are the highest-value candidate.
